...
/// Gather the user's own email addresses from the unlocked vault: login
/// usernames that are emails, identity `email` fields, plus the account email.
...
/// Scan every account email in the vault (capped at `MAX_VAULT_EMAILS` per run).
/// Opt-in. Throttled to stay under the provider's rate limit.
pub async fn scan_vault(state: &AppState) -> AgateResult<DarkWebReport> {
    require_consent(state).await?;
    let emails = collect_account_emails(state).await?;
    let skipped = emails.len().saturating_sub(MAX_VAULT_EMAILS);
    let client = http_client()?;

    let mut accounts = Vec::new();
    for (i, email) in emails.into_iter().take(MAX_VAULT_EMAILS).enumerate() {
        if i > 0 {
            tokio::time::sleep(THROTTLE).await;
        }
        accounts.push(fetch_email(&client, &email).await?);
    }

    let total_breaches = accounts.iter().map(|a| a.breaches.len()).sum();
    let clean = accounts.iter().filter(|a| a.breaches.is_empty()).count();
    Ok(DarkWebReport { accounts, total_breaches, clean, skipped })
}
