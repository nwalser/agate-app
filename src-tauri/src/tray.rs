...
/// Top-left corner for the popup so it hugs the click point and stays fully on
/// the monitor (all physical px): horizontally centered on the click, opening
/// away from the taskbar edge (above when the click is in the lower monitor
/// half — Windows taskbar — below when in the upper half — macOS menu bar),
/// then clamped inside the monitor with a margin. Pure, unit-tested.
fn popup_position(
    cursor: (f64, f64),
    monitor_pos: (i32, i32),
...
mod tests {
    ...
    const MON_SIZE: (u32, u32) = (2560, 1440);
    const WIN: (u32, u32) = (380, 520);

    #[test]
    fn bottom_taskbar_opens_above_centered_on_click() {
        let (x, y) = popup_position((2000.0, 1430.0), MON_POS, MON_SIZE, WIN);
        assert_eq!(x, 2000 - 190);
        assert_eq!(y, 1430 - 520 - 8);
    }

    #[test]
    fn top_menubar_opens_below_click() {
        let (x, y) = popup_position((1280.0, 5.0), MON_POS, MON_SIZE, WIN);
    ...
    fn clamps_inside_right_edge() {
        let (x, _) = popup_position((2550.0, 1430.0), MON_POS, MON_SIZE, WIN);
        assert_eq!(x, 2560 - 380 - 8);
    }

    #[test]
    fn clamps_on_negative_origin_secondary_monitor() {
        let (x, y) = popup_position((-30.0, 1430.0), (-2560, 0), MON_SIZE, WIN);
        assert_eq!(x, -2560 + 2560 - 380 - 8);
        assert_eq!(y, 1430 - 520 - 8);
    }

    #[test]
    fn window_taller_than_monitor_pins_to_top_without_panicking() {
